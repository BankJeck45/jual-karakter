...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    // DALL-E 3 only supports n=1, so we make multiple calls for n>1
    const results = [];
    const callsNeeded = Math.min(n, 4); // max 4 images
...
app.listen(PORT, () => {
  console.log(`🚀 DALL·E API server running on http://localhost:${PORT}`);
});
